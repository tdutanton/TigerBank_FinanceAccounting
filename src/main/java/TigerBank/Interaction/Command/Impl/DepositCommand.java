...
public class DepositCommand extends BaseCommand {
  ...
  public DepositCommand(CommandContext ctx) {
    super(ctx);
  }

  @Override
  public void execute() {
    withTiming("пополнение", () -> {
      ...
      try {
        if (description != null) {
          ctx.bank().depositToAccount(accountName, amount, category, description);
        } else {
          ctx.bank().depositToAccount(accountName, amount, category);
        }
        ctx.logger().info(String.format("Счёт '%s' пополнен на %.2f с категорией '%s'%s",
